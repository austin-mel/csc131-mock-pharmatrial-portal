...
export function buildAppointment(draft: AppointmentDraft): Appointment {
  ...
  return {
    ...
    type: draft.type,
    dose: isDose,
    bloodTestLevel: Number.isFinite(draft.bloodTestLevel) ? draft.bloodTestLevel : null,
    adverseEvents: draft.adverseEvents.map((event) => event.trim()).filter(Boolean),
    note: draft.note.trim() || "No notes.",
  };
}

export function logAppointment(enrollment: TrialEnrollment, dosesPerPatient: number, draft: AppointmentDraft): Appointment {
