...
export function calculateTreatmentPatientCounts(
...
export function calculateBatch(
    ...
    const dosesPerPatient = Math.max(
        0,
        Math.floor(Number(trial.dosesPerPatient) || 0),
    );
    const { treatmentPatients, placeboPatients } =
