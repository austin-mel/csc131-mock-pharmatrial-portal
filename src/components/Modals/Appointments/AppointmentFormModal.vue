...
import type { AppointmentType, Patient, Trial, TrialEnrollmentMap } from "@/types";
...
function save() {
  if (auth.selectedPortalId !== "jh-doctor") {
    ui.pushToast("Only Jane Hopkins doctors can log appointments.", "error");
    return;
  }
  ...
  const saved = trials.logAppointment(props.trial.id, patientId.value, {
    date: date.value,
    time: time.value,
    type: type.value,
    bloodTestLevel: bloodTestLevel.value === null || bloodTestLevel.value === "" || !Number.isFinite(parsedBloodLevel)
      ? null
      : parsedBloodLevel,
  ...
  if (!saved) {
    ui.pushToast("Appointment could not be saved for this patient.", "error");
    return;
  }
  ...
  <ModalShell :open="open" title="Log Appointment" @close="$emit('close')">
    ...
    <FormField label="Appointment Type">
      <FormSelect v-model="type">
        <option v-for="appointmentType in appointmentTypes" :key="appointmentType">
          {{ appointmentType }}
        </option>
      </FormSelect>
    </FormField>
    ...
    <FormField label="Adverse Events">
      <div class="space-y-2">
        <div v-for="(_, index) in adverseEvents" :key="index" class="flex gap-2">
          <FormInput v-model="adverseEvents[index]" placeholder="e.g. Mild fatigue" />
          <button
        ...
        <ActionButton @click="addAdverseEvent">Add Adverse Event</ActionButton>
      </div>
    </FormField>
    ...
    <template #footer>
      ...
      <ActionButton variant="jh" :disabled="!patientOptions.length" @click="save">Save Appointment</ActionButton>
    </template>
  </ModalShell>
</template>
