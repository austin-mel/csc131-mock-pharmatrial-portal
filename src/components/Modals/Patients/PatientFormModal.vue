...
function save() {
    ...
    trials.enrollPatient(
        props.trial.id,
        patient.id,
        checkEligibility(patient, props.trial),
    );
    ui.pushToast(
        props.patient
            ? "Patient record updated."
            : "Patient added and enrolled.",
        "success",
    );
    emit("close");
}
...
<template>
    <ModalShell
        ...
        :title="
            patient
                ? 'Edit Patient Record'
                : 'Add New Patient & Enroll in Trial'
        "
        wide
        @close="$emit('close')"
    >
        <div
            class="mb-2 mt-1 font-mono text-[10px] uppercase tracking-[.12em] text-muted"
        >
            Demographics
        </div>
        <FormRow>
        ...
        <FormRow>
            <FormField label="Date of Birth"
                ><FormInput v-model="dob" type="date"
            /></FormField>
        ...
        <FormField label="ICD-10 Codes"
            ><FormInput v-model="icdCodes" placeholder="B20, Z21"
        /></FormField>
        <FormRow>
            <FormField label="Height (cm)"
                ><FormInput v-model="height" type="number"
            /></FormField>
            <FormField label="Weight (kg)"
                ><FormInput v-model="weight" type="number"
            /></FormField>
        </FormRow>
        <FormRow>
            <FormField label="Blood Pressure"
                ><FormInput v-model="bp" placeholder="118/76"
            /></FormField>
            <FormField label="O2 Saturation (%)"
                ><FormInput v-model="oxygen" type="number"
            /></FormField>
        </FormRow>
        <FormField label="Temperature"
            ><FormInput v-model="temperature" placeholder="98.6F"
        /></FormField>
        <FormField label="Current Medications"
            ><FormTextarea v-model="meds"
        /></FormField>
        <FormField label="Allergies"
            ><FormTextarea v-model="allergy"
        /></FormField>
        <template #footer>
