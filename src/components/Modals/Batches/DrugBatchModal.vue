...
const validTreatmentPct = computed(
    () =>
        Number.isFinite(parsedTreatmentPct.value) &&
        parsedTreatmentPct.value >= 10 &&
        parsedTreatmentPct.value <= 90,
);
const draftTrial = computed<Trial>(() => ({
    ...props.trial,
    treatmentPct: validTreatmentPct.value
        ? parsedTreatmentPct.value
        : (props.trial.treatmentPct ?? 50),
}));
const batch = computed(() =>
    calculateBatch(props.eligibleCount, draftTrial.value),
);
const canSubmit = computed(
    () =>
        auth.selectedPortalId === "bavaria" &&
        canSubmitBatch(props.trial) &&
        validTreatmentPct.value,
);
...
watch(
    ...
    (open) => {
        if (!open) return;
        batchRef.value =
            props.trial.batchRef ?? `BAV-${props.trial.id}-BATCH-001`;
        treatmentPct.value = props.trial.treatmentPct ?? 50;
        manufactureDate.value =
            props.trial.manufactureDate ??
            new Date().toISOString().slice(0, 10);
        lotNumber.value =
            props.trial.lotNumber ??
            `LOT-${props.trial.id.replace(/\D/g, "") || "001"}`;
        shippingNotes.value =
            props.trial.shippingNotes ??
            "Store at 2-8C. Handle with care. Do not freeze.";
    },
);
...
function submit() {
    ...
    if (!submitted) {
        ui.pushToast(
            "Batch cannot be submitted until FDA and Jane Hopkins approvals are complete.",
            "error",
        );
        return;
    }
...
<template>
    <ModalShell :open="open" title="Submit Drug Batch" @close="$emit('close')">
        ...
        >
            Total vials are calculated from eligible patients and doses per
            patient. Submitting a new batch clears any stale FDA assignments.
        </p>
        ...
        <div
            class="mt-3 grid grid-cols-5 gap-3 max-[980px]:grid-cols-2 max-[640px]:grid-cols-1"
        >
            <StatCard label="Total Vials">{{ batch.totalVials }}</StatCard>
            <StatCard label="Treatment Patients">{{
                batch.treatmentPatients
            }}</StatCard>
            <StatCard label="Placebo Patients">{{
                batch.placeboPatients
            }}</StatCard>
        ...
        <template #footer>
            ...
            <ActionButton variant="bav" :disabled="!canSubmit" @click="submit"
                >Submit Batch</ActionButton
            >
        </template>
    </ModalShell>
</template>
