...
function notify() {
    ...
    if (!saved) {
        ui.pushToast(
            "FDA notification is locked until assignments are locked and all eligible patients are fully dosed.",
            "error",
        );
        return;
    }
...
<template>
    <ModalShell
        :open="open"
        title="Notify FDA of Completion"
        @close="$emit('close')"
    >
        <p
            class="mb-4 rounded-md border border-rule bg-bg px-3 py-2 text-sm text-muted"
        >
            Transmit anonymized completion results to FDA. This locks the Jane
            Hopkins patient roster for this trial.
        </p>
