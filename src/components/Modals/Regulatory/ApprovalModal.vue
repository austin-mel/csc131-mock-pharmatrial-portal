...
const enrollmentCount = computed(() =>
    props.trial ? Object.keys(trials.enrollmentsFor(props.trial.id)).length : 0,
);
const jhApprovalBlocked = computed(
    () => auth.selectedPortalId === "jh-admin" && enrollmentCount.value === 0,
);
const canReview = computed(
    () =>
        auth.selectedPortalId === "fda" || auth.selectedPortalId === "jh-admin",
);
...
function approve() {
    ...
    const eligibility =
        auth.selectedPortalId === "fda" ? eligibilityDraft() : undefined;
    const approved = trials.approveTrial(
        props.trial.id,
    ...
    if (!approved) {
        ui.pushToast(
            jhApprovalBlocked.value
                ? "Assign at least 1 patient before approving."
                : "Trial cannot be approved from its current state.",
            "error",
        );
        return;
    }
    ...
    if (eligibility) {
        ...
        if (trial) {
            Object.keys(trials.enrollmentsFor(trial.id)).forEach(
                (patientId) => {
...
function reject() {
    ...
    if (!rejected) {
        ui.pushToast(
            "Trial cannot be rejected from its current state.",
            "error",
        );
        return;
    }
...
<template>
    <ModalShell :open="open" title="Review Trial" @close="$emit('close')">
        <DetailGrid v-if="trial" :items="items" />
        <template v-if="trial && auth.selectedPortalId === 'fda'">
            <div
                class="mb-2 mt-4 font-mono text-[10px] uppercase tracking-[.12em] text-muted"
            >
            ...
            <FormField label="Incompatible Medications">
                <FormInput
                    v-model="incompatMeds"
                    placeholder="Tenofovir, Abacavir"
                />
            </FormField>
        </template>
        ...
        >
            Assign at least 1 patient before Jane Hopkins can approve this
            trial.
        </div>
        <template #footer>
            <div
