...
import { ActionButton, ModalShell } from "@/components";
...
function disclose() {
    const saved = trials.discloseTrial(
        props.trial.id,
    ...
    if (!saved) {
        ui.pushToast(
            "Disclosure is locked until FDA notification, locked assignments, and complete dosing are all present.",
            "error",
        );
        return;
    }
...
<template>
    <ModalShell
        :open="open"
        title="Disclose Trial Assignments"
        @close="$emit('close')"
    >
        <p
            class="mb-4 rounded-md border border-[#f0b8b0] bg-[#fdecea] px-3 py-2 text-sm text-[#9b1c11]"
        >
            Publish the final report and reveal treatment assignments to all
            parties. This marks the trial complete.
        </p>
        <div class="grid grid-cols-3 gap-3 text-sm max-[640px]:grid-cols-1">
            <div class="rounded-md border border-rule px-3 py-2">
                ...
                <strong>{{
                    trial.notifiedFDA ? "Received" : "Pending"
                }}</strong>
            </div>
            <div class="rounded-md border border-rule px-3 py-2">
                <div
                ...
                <strong>{{
                    trial.assignmentsLocked ? "Locked" : "Pending"
                }}</strong>
            </div>
            <div class="rounded-md border border-rule px-3 py-2">
                <div
