...
const pills = computed(() => [
    ...
    {
        value: `${props.completedCount}/${props.eligibleCount}`,
        label: "Fully Dosed",
    },
]);
</script>
...
<template>
    ...
    >
        <div class="min-w-0">
            ...
            <div
                class="mb-[3px] break-words font-serif text-[22px] leading-[1.2] max-[640px]:text-[20px]"
            >
                {{ trial.name }}
            </div>
            ...
            <div
                class="flex flex-wrap gap-2 max-[480px]:grid max-[480px]:grid-cols-2"
            >
                <span
                    v-for="pill in pills"
                    :key="pill.label"
                ...
                >
                    <strong class="truncate font-sans text-xs text-white/90">{{
                        pill.value
                    }}</strong>
                    {{ pill.label }}
                </span>
