...
const allDosed = computed(() =>
    allEligibleDosed(
        props.trial,
        patients.patients,
        trials.enrollmentsFor(props.trial.id),
    ),
);
const review = computed(() =>
    needsWorkflowReviewTag(props.trial, auth.selectedPortalId, allDosed.value),
);
</script>
...
<template>
    ...
    >
        ...
        <div class="mt-[5px] flex min-w-0 flex-wrap gap-1">
            ...
            <StatusBadge v-if="review" tone="red"> Needs Review </StatusBadge>
        </div>
    </button>
</template>
