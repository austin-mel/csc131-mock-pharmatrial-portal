...
import { ActionButton, DataCard, DetailGrid, StatCard } from "@/components";
...
const submittedItems = computed(() => [
    ...
    {
        label: "Treatment / Placebo Split",
        value: `${props.trial.treatmentPct ?? 50}% / ${100 - (props.trial.treatmentPct ?? 50)}%`,
    },
...
<template>
    <div>
        <div class="mb-5 flex flex-wrap items-start justify-between gap-2.5">
            <div>
                ...
                <div class="mt-0.5 text-[13px] text-muted">
                    Submit labeled vials to the FDA for {{ trial.name }}.
                </div>
            </div>
            <ActionButton
        ...
        <div
            v-if="!allApproved"
            class="mb-5 rounded-md border border-[#f3d7a2] bg-[#fff8e8] px-3 py-2 text-sm text-[#8a5a00]"
        >
            Drug batch cannot be submitted until both Jane Hopkins and the FDA
            approve this trial.
        </div>
        <template v-if="trial.batchSubmitted">
            <div
                class="mb-5 rounded-md border border-[#b8dfc5] bg-[#e6f4ec] px-3 py-2 text-sm text-[#1e7e4e]"
            >
                Batch {{ trial.batchRef }} has been submitted and received by
                the FDA.
            </div>
        ...
        <template v-else>
            <DataCard title="Batch Calculation">
                <div
                    class="m-0 grid grid-cols-4 gap-3 px-[18px] py-4 max-[780px]:grid-cols-2 max-[520px]:grid-cols-1"
                >
                    <StatCard label="Eligible Patients">{{
                        eligibleCount
                    }}</StatCard>
                    <StatCard label="Doses Each">{{
                        trial.dosesPerPatient
                    }}</StatCard>
                    <StatCard label="Total Vials">{{
                        batch.totalVials
                    }}</StatCard>
                    <StatCard label="Patient Split"
