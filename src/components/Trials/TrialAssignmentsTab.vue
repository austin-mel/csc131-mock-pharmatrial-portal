...
import { ActionButton, AssignmentPreviewTable, DataCard } from "@/components";
...
const batch = computed(() =>
    calculateBatch(props.patients.length, props.trial),
);
</script>
...
<template>
    <div>
        <div class="mb-5 flex flex-wrap items-start justify-between gap-2.5">
            <div>
                ...
                <div class="mt-0.5 text-[13px] text-muted">
                    Private FDA mapping until disclosure.
                </div>
            </div>
            <ActionButton
        ...
        >
            This table is strictly confidential. Assignments remain hidden from
            all other parties until end-of-trial disclosure.
        </div>
        <div
            v-if="!trial.batchSubmitted"
            class="rounded-md border border-rule bg-bg px-[18px] py-5 text-sm text-muted"
        >
        ...
        <template v-else>
            ...
            <DataCard title="Batch Details">
                <div
                    class="grid grid-cols-4 gap-3.5 px-[18px] py-4 max-[780px]:grid-cols-2 max-[520px]:grid-cols-1"
                >
                    <div>
                        <div
                    ...
                    <div>
                        ...
                        <div class="text-xs text-muted">
                            {{ batch.treatmentPatients }} patients
                        </div>
                    </div>
                    <div>
                        <div
                        ...
                        <div class="text-xs text-muted">
                            {{ batch.placeboPatients }} patients
                        </div>
                    </div>
                </div>
            </DataCard>
