...
const canAdd = computed(() =>
    canAddPatients(props.trial, auth.selectedPortalId),
);
...
const canEditPatients = computed(
    () => pii.value && props.trial.status !== "complete",
);
const canDeletePatients = computed(
    () =>
        auth.selectedPortalId === "jh-admin" &&
        props.trial.status !== "complete",
);
const rows = computed(() =>
    props.patients.filter((patient) => props.enrollments[patient.id]),
);
const filteredRows = computed(() => {
    const q = query.value.trim().toLowerCase();
    if (!q) return rows.value;
    return rows.value.filter((patient) =>
        [patient.name, patient.id, patient.icdCodes.join(" ")].some((value) =>
...
const eligibleCount = computed(
    () =>
        rows.value.filter((patient) => props.enrollments[patient.id]?.eligible)
            .length,
);
    ...
    () =>
        rows.value.filter(
            (patient) =>
                ...
                (props.enrollments[patient.id]?.doses ?? 0) >=
                    props.trial.dosesPerPatient,
        ).length,
);
const totalDoses = computed(() =>
    totalDosesGiven(rows.value, props.enrollments),
);
...
<template>
    <div>
        <div class="mb-5 flex flex-wrap items-start justify-between gap-2.5">
            <div>
                <div class="font-serif text-2xl font-normal">
                    {{ pii ? "Patients" : "Patients (Anonymized)" }}
                </div>
                <div class="mt-0.5 text-[13px] text-muted">
                    {{
                        pii
                            ? "Full PII visible for Jane Hopkins doctor access."
                            : "Patient UUIDs and clinical flags only. No PII at this access level."
                    }}
                </div>
            </div>
            <div
            ...
            >
                ...
                <ActionButton
                    v-if="canCsv"
                    @click="ui.showModal('patient-csv')"
                >
                ...
                <ActionButton
                    v-if="canAdd"
                    variant="jh"
                    @click="ui.showModal('patient-form')"
                >
                    <SvgIcon name="add" />
                    Add Patient
                </ActionButton>
        ...
        <div
            v-if="!canAdd && pii"
            class="mb-5 rounded-md border border-rule bg-bg px-3 py-2 text-sm text-muted"
        >
            {{
                trial.notifiedFDA
                    ? "Patient roster is locked because results have been sent to the FDA."
                    : "Patient enrollment is locked once a trial is active or completed."
            }}
        </div>
        <div class="mb-5 grid grid-cols-2 gap-3 max-[640px]:grid-cols-1">
            <StatCard label="Enrolled">{{ rows.length }}</StatCard>
            <StatCard label="Completed Doses"
                >{{ totalDoses }}/{{ requiredDoses }}</StatCard
            >
        </div>
        <PatientTable
            v-if="pii"
        ...
        <PatientAnonymizedTable
            ...
            :show-tracking="
                auth.selectedPortalId === 'bavaria' || trial.disclosed
            "
