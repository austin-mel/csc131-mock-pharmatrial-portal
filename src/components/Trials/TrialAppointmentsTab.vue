...
import TrialAppointmentsTabLogTable from "@/components/Trials/TrialAppointmentsTabLogTable.vue";
...
const eligiblePatients = computed(() => props.patients.filter((patient) => props.enrollments[patient.id]?.eligible));
...
const rows = computed<AppointmentLogRow[]>(() =>
  filteredPatients.value
    .flatMap((patient) =>
      (props.enrollments[patient.id]?.appointments ?? []).map((appointment) => ({
        ...appointment,
...
const canLogAppointment = computed(() => auth.selectedPortalId === "jh-doctor" && props.trial.status !== "complete");
</script>
  ...
  <div>
    <div class="mb-5 flex flex-wrap items-start justify-between gap-2.5">
      <div>
        ...
        <div class="mt-0.5 text-[13px] text-muted">All visits and dose administrations for {{ trial.name }}.</div>
      </div>
