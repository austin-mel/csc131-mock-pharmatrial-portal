  ...
  <div>
    <DataCard title="Visit Log">
      ...
      <div class="hidden gap-3 p-3 max-[520px]:grid">
        <div v-if="!pagedRows.length" class="rounded-md border border-rule bg-bg p-3 text-sm text-muted">
        ...
        >
          <div class="mb-2 flex items-start justify-between gap-3">
            <div class="min-w-0">
              <strong class="block break-words text-[15px] text-ink">{{ row.type }}</strong>
              <div class="font-mono text-xs text-fda">{{ row.date }} <span v-if="row.time">{{ row.time }}</span></div>
            </div>
            <StatusBadge :tone="row.dose ? 'green' : 'gray'">
          ...
          <div class="grid gap-2 text-[13px]">
            <div>
              <span class="font-mono text-[10px] uppercase tracking-[.12em] text-muted">Patient</span>
              <div class="text-ink">
                <strong>{{ row.patientName }}</strong>
                <div class="font-mono text-xs text-fda">{{ row.patientId }}</div>
              </div>
            ...
            <div>
              <span class="font-mono text-[10px] uppercase tracking-[.12em] text-muted">Blood Test</span>
              <div class="text-ink">{{ row.bloodTestLevel ?? "Not recorded" }}</div>
            </div>
            <div>
              <span class="font-mono text-[10px] uppercase tracking-[.12em] text-muted">Adverse Events</span>
              <div class="text-ink">{{ row.adverseEvents.length ? row.adverseEvents.join(", ") : "None" }}</div>
            </div>
            <div>
              <span class="font-mono text-[10px] uppercase tracking-[.12em] text-muted">Notes</span>
              <div class="break-words text-ink">{{ row.note }}</div>
            </div>
          </div>
        </article>
      </div>
