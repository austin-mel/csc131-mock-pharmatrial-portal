...
export function buildReportRowDisplay(
    ...
    const patientName =
        patient && showPii ? patient.name : maskPatientName(row.patientId);
    ...
    return {
        ...
        adverseEvents: row.adverseEvents.length
            ? row.adverseEvents.join(", ")
            : "None",
    };
}

export function buildAnonymizedReportRowDisplay(
    row: ReportRow,
): ReportRowDisplay {
